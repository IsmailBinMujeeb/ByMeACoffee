...
export const addItem = async (req, res) => {
  try {
    ...
    if (!itemName?.trim() || typeof Number(price) !== "number" || !filePath)
    ...
    if (!newItem)
      return res.status(403).json({ message: "item creation failed" });

    return res
      .status(201)
      .json({ message: "ok", success: true, item: newItem });
  } catch (error) {
    console.log(error);
    return res
      .status(500)
...
export const getAllItems = async (req, res) => {
  try {
    ...
    const items = await menuItemModel.paginate(
      {},
...
export const updateItem = async (req, res) => {
  try {
    ...
    const newItem = await menuItemModel.findByIdAndUpdate(
      _id,
      {
        itemName,
        price,
...
export const removeItem = async (req, res) => {
  try {
    ...
    if (!deletedItem)
      return res.status(403).json({ message: "item deletion failed" });

    return res
      .status(200)
      .json({ message: "ok", success: true, item: deletedItem });
  } catch (error) {
    console.log(error);
    return res
      .status(500)
